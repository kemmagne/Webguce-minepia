...
public class ATMRegistrationRepository extends AbstractFacade<ATMRegistration> {
    @PersistenceContext(
            unitName = "Partner-ejb-unity"
    )
    private EntityManager em;

    public ATMRegistrationRepository() {
        super(ATMRegistration.class);
    }
    ...
    public List<ATMRegistration> findByDomain(String domain) {
        ...
        String qb = "Select distinct i from ATMRegistration i where i.deleted=false and i.active = true "+domain ;
        System.out.println(qb);
        TypedQuery<ATMRegistration> query = em.createQuery(qb, ATMRegistration.class);
        return query.getResultList();
    }

    public int searchCount(SearchFilter filter) {
        String qb = "Select count(distinct i) from ATMRegistration i where i.deleted=false ";
    ...
    public List<ATMRegistration> search(SearchFilter filter, int start, int count, String orderField, String order) {
        ...
        String qb = "Select distinct i from ATMRegistration i where i.deleted=false";
        qb = addConditions(qb, filter);
